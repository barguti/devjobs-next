    ...
    return (
        <main className="min-h-screen bg-[#0B111D] flex items-center justify-center p-6">
            <div className="max-w-4xl w-full">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    >
                        <div className={`w-14 h-14 rounded-xl flex items-center justify-center mb-6 transition-colors ${
                            role === 'developer' ? 'bg-brand-blue text-white' : 'bg-[#19202D] text-[#9DAEC2]'
                        }`}>
                            <User size={32} />
                        </div>
                        <h3 className="text-white text-2xl font-bold mb-2">{"I'm a Developer"}</h3>
                        <p className="text-[#9DAEC2] mb-6">
                            I want to find my next dream job and manage my applications.
                        </p>
                        <span className={`flex items-center gap-2 font-bold transition-all ${
                            role === 'developer' ? 'text-brand-blue' : 'text-gray-500 group-hover:text-gray-300'
                        }`}>
                    ...
                    >
                        <div className={`w-14 h-14 rounded-xl flex items-center justify-center mb-6 transition-colors ${
                            role === 'employer' ? 'bg-brand-blue text-white' : 'bg-[#19202D] text-[#9DAEC2]'
                        }`}>
                            <Briefcase size={32} />
                        </div>
                        <h3 className="text-white text-2xl font-bold mb-2">{"I'm an Employer"}</h3>
                        <p className="text-[#9DAEC2] mb-6">
                            I want to post jobs and find the best talent for my company.
                        </p>
                        <span className={`flex items-center gap-2 font-bold transition-all ${
                            role === 'employer' ? 'text-brand-blue' : 'text-gray-500 group-hover:text-gray-300'
                        }`}>
                            Select this option <ArrowRight size={18} />
                        </span>
                    </div>
                ...
                {/* Botón de Continuar (Solo aparece si eligió algo) */}
